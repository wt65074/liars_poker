    ...
    def handleData(self, dataType, data):
        ...
        if dataType == 0:
            ...
            for string in data.split("|"):
                ...
                self.factory.gameState.deviceTokens += string.split("+")[0]

            #Save data as a json
            self.factory.gameState.dumpPlayerDeviceTokens()
...
class GameState(object):
    ...
    def dumpPlayerDeviceTokens(self):

        with open('config.json') as json_data_file:
            data = json.load(json_data_file)

        with open(data['storageFolder'] + "/players.json", 'w') as outfile:
            data = {'deviceTokens' : self.deviceTokens}
            json.dump(data, outfile)

        print "Data Dump"
        import subprocess
        subprocess.call("ruby " + data["pushNotificationScript"] + " -p " + str(self.port) + " -j ", data['storageFolder'] + "/players.json", shell=True)
